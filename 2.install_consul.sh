...
if [[ ! ${CONSUL_VERSION+x} ]]; then CONSUL_VERSION='1.5.1' ; fi ; # // VERSIONS: "1.7.0' for regular release version - '1.6.3+ent' for Enterprise
...
if [[ ! ${PATH_INSTALL+x} ]]; then PATH_INSTALL="$(realpath ~/consul_installs)" ; fi ; # // where consul install files will be
...
if [[ ${CONSUL_VERSION} == '' ]] ; then
	CONSUL_VERSION=$(curl -s ${URL_CONSUL} | grep '<a href="/consul/' | grep -v '-beta' | head -n 1 | grep -E -o '([0-9]{1,3}[\.]){2}[0-9]{1,3}' | head -n 1) ;
	if [[ ${CONSUL_VERSION} == '' ]] ; then
		printf '\nERROR: Could not determine valid / current consul version to download.\n' ;
		exit 1 ;
	fi ;
fi ;

set +e ; CHECK=$(consul --version 2>&1) ; set -e ; # // maybe required consul version is already installed.
if [[ ${CHECK} == *"v${CONSUL_VERSION}"* ]] && [[ (($# == 0)) || $1 != "-f" ]] ; then printf "Consule v${CONSUL_VERSION} already installed; Use '-f' to force this script to run anyway.\nNo action taken.\n" && exit 0 ; fi ;
...
function donwloadUnpack()
{
	...
	if wget -qc ${URL} && wget -qc ${URL2} ; then
		if shasum -a 256 -c consul_${CONSUL_VERSION}_SHA256SUMS --ignore-missing 2>&1>/dev/null ; then
...
case "$(uname -ar)" in
	Darwin*)
		...
		if which brew > /dev/null ; then
			printf 'Consider: "brew install consul" since you have HomeBrew availble.\n' ;
		else :; fi ;
...
function sudoSetup()
{
	...
	if ! id -u ${USER_CONSUL} &>/dev/null ; then 
		...
		printf 'USER: consul - already present.\n' ;
	fi ;
...
' > ${PATH_CONSUL_CONFIG} && chmod 640 ${PATH_CONSUL_CONFIG} ; then :;
		    ...
		    printf "\nERROR: Unable to create config: ${PATH_CONSUL_CONFIG}.\n" ; exit 1 ;
	    fi ;
	else
		if [[ ${SETUP,,} == *'client'* ]] ; then printf "CONSUL Conifg: ${PATH_CONSUL_CONFIG} - already present.\n" ; fi ;
	fi ;
...
' > ${PATH_CONSUL_CONFIG_SERVER} ; then :;
			...
			printf "\nERROR: Unable to create ${PATH_CONSUL_CONFIG_SERVER}.\n" ; exit 1 ;
		fi ;

	else
		if [[ ${SETUP,,} == *'server'* ]] ; then printf "CONSUL Server conifg: ${PATH_CONSUL_CONFIG_SERVER} - already present.\n" ; fi ;
	fi ;
	...
	if ! [[ ${LICENSE} == "" ]] && [[ ${SETUP,,} == *'server'* ]] ; then
		...
		if [[ ${LICENSE_ATM} == *": temporary"* ]] ; then
			...
			printf "CONSUL LICNESE: Already present & ${LICENSE_EXP}\n" ;
		fi ;
	fi ;
}
URL="${URL}${FILE}" ;
